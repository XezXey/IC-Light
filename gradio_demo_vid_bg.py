...
@torch.inference_mode()
def process_relight_vid(input_fg, input_bg_vid, prompt, image_width, image_height, num_samples, seed, steps, a_prompt, n_prompt, cfg, highres_scale, highres_denoise, bg_source):
    input_fg, matting = run_rmbg(input_fg)
    # Load video
    input_bg_frames, _, _ = torchvision.io.read_video(input_bg_vid)
    print(input_bg_frames.shape)
    print(torch.max(input_bg_frames), torch.min(input_bg_frames))
    
    res = []
    fg = []
    bg = []
    for input_bg in input_bg_frames:
        ...
        for x in results:
            ...
            print(np.max(x))
            print(np.min(x))
        print(extra_images[0].shape)
        print(extra_images[1].shape)
    ...
    print(res.shape)
    torchvision.io.write_video('output.mp4', res, 1)
    torchvision.io.write_video('fg.mp4', fg, 1)
    torchvision.io.write_video('bg.mp4', bg, 1)
    return res
    # return results + extra_images
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                ...
                input_bg_vid = gr.Video(source='upload', label="Background Video", height=480)
            ...
            relight_vid_button = gr.Button(value="Relight w/ bg video")
    ...
    ips_vid = [input_fg, input_bg_vid, prompt, image_width, image_height, num_samples, seed, steps, a_prompt, n_prompt, cfg, highres_scale, highres_denoise, bg_source]
    relight_button.click(fn=process_relight, inputs=ips, outputs=[result_gallery])
    relight_vid_button.click(fn=process_relight_vid, inputs=ips_vid, outputs=[result_gallery])
